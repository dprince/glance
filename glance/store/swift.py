...
class StoreLocation(glance.store.location.StoreLocation):
    ...
    When using multi-tenant a URI might look like this:
    ...
    def get_uri(self):
        ...
        if auth_or_store_url.startswith('http://'):
            auth_or_store_url = auth_or_store_url[7:]
        elif auth_or_store_url.startswith('https://'):
            auth_or_store_url = auth_or_store_url[8:]
    ...
    @property
    def swift_auth_or_storage_url(self):
        """
    ...
    def _swift_connection_for_location(self, loc):
        if loc.user:
            return self._make_swift_connection(
                loc.swift_auth_or_storage_url, loc.user, loc.key)
        else:
            if self.multi_tenant:
                return self._make_swift_connection(
                    None, self.user, None,
                    storage_url=loc.swift_auth_or_storage_url,
                    token=self.token)
            else:
